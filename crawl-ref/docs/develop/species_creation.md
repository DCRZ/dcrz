...
| name | `string` | **Yes** | Species name, like 'Sludge Dwarf' or 'Felid'. Required. Must be unique. |
...
| mutations | `map {xl: {mut: amt}}` | No | Innate mutations for the species, and the XL it gains them. The map is keyed by XL, then each XL is another map containing the mutation enum name keying the number of levels to change. Look at Sludge Dwarf for a good example. |
